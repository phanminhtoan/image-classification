...
async function loadModel() {
	console.log("model loading mobilenet model ...");
	...
	model = await tf.loadLayersModel('model_imagenet/model.json');
...
function loadImageLocal() {
	console.log("Click into selected file image");
  	...
  	document.getElementById("prediction").innerHTML = "Click predict to find the result!";
...
async function predictImage(){
	...
	document.getElementById("prediction").innerHTML = "My prediction result <br><b>" + results[0].className + "</b>";
	...
	results.forEach(function (p) {
		...
		li.innerHTML = "Predict: " + p.className + " with probability is " + (p.probability*100).toFixed(3)+ "%"; 
		ul.appendChild(li);
	})
